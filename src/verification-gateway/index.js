...
const TRILLIAN_URL =
  process.env.TRILLIAN_LOG_URL || "http://trillian-log-server:3000";
...
app.post(
  ...
  (req, res, next) => {
    if (
      !req.user ||
      (req.user.role !== "verifier" && req.user.role !== "verifier_system")
    ) {
...
app.post("/webhooks/topic/present_proof/", async (req, res, next) => {
  try {
    ...
    if (state === "verified") {
      const sessionRaw = await redisClient.get(
        `session:${presentation_exchange_id}`,
      );
      if (sessionRaw && (verified === "true" || verified === true)) {
        // Fire and forget logging
        handleSuccessfulVerification(presentation_exchange_id, req.body).catch(
          console.error,
        );
  ...
  try {
    await axios.post(`${TRILLIAN_URL}/api/v1/log/entries`, {
      ...
      spec: {
        signature: {
          content: leafData,
          publicKey: { content: "base64_pub_key" },
        },
      },
    });
  } catch (e) {
