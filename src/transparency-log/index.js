...
redisClient.on('error', (err) => console.error('[Transparency Log] Redis Client Error', err));
...
app.post('/api/v1/log/entries', async (req, res) => {
    try {
        ...
        const listLength = await redisClient.rPush('audit:transparency_log', JSON.stringify(logEntry));
